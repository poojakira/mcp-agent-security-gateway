...
EXFILTRATION_INTENT_PATTERNS = [
    (
        r"(send|post|upload|transmit|forward|relay).*(secret|key|token|password|credential)",
        "data_exfil_intent",
    ),
    (
        r"(hidden|covert|stealth|silent|shadow).*(channel|recipient|destination|endpoint)",
        "covert_channel",
    ),
]
DANGEROUS_FIELD_SEMANTICS = {
    "extra_recipients": 90,
    "additional_destinations": 85,
    "mirror_addresses": 90,
    "webhook_notify": 60,
    "callback_url": 50,
    "notification_endpoint": 55,
    "audit_copy": 70,
}
...
class SemanticIntentAnalyzer:
    ...
    ) -> tuple[bool, list[SemanticFinding]]:
            ...
            if key.lower().replace("-", "_").replace(" ", "_") in EMAIL_RECIPIENT_SYNONYMS:
                findings.append(
                    SemanticFinding(
                        ...
                        confidence=0.95,
                        description=f"Field '{key}' is BCC synonym",
                ...
                if re.search(pattern, text, re.IGNORECASE):
                    findings.append(
                        SemanticFinding(
                            intent=name,
                            confidence=0.8,
                            description=f"Exfil pattern: {name}",
                            evidence=[text[:100]],
                            severity=80,
                        )
                    )
            ...
            for df, sev in DANGEROUS_FIELD_SEMANTICS.items():
                if df in kl or kl in df:
                    ...
                    if val:
                        findings.append(
                            SemanticFinding(
                                intent="suspicious_field",
                                confidence=0.75,
                                description=f"Dangerous field: '{key}'",
                                evidence=[f"field='{key}'"],
                                severity=sev,
                            )
                        )
            ...
            if len(text) > 20 and re.match(r"^[A-Za-z0-9+/=]+$", text):
                try:
                    ...
                    if "@" in decoded and "." in decoded:
                        findings.append(
                            SemanticFinding(
                                intent="encoded_email",
                                confidence=0.9,
                                description="Base64 email detected",
                                evidence=["decoded has email"],
                                severity=90,
                            )
                        )
                except Exception:
        ...
        if all_keys & email_indicators:
            ...
            for ef in extra:
                if any(
                    ...
                    findings.append(
                        SemanticFinding(
                            intent="email_extra_recipient",
                            confidence=0.85,
                            description=f"Email extra field: '{ef}'",
                            evidence=[ef],
                            severity=85,
                        )
                    )
    ...
    def _all_strings(self, obj: Any) -> list[str]:
        ...
        if isinstance(obj, str):
            ...
            for v in obj.values():
                s.extend(self._all_strings(v))
        elif isinstance(obj, list):
            for i in obj:
                s.extend(self._all_strings(i))
        return s
