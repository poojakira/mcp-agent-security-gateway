...
class NetworkEgressPolicy:
    ...
    def evaluate(
        ...
        if not matching_rules:
            decision = EgressDecision(
                ...
                + (": default_deny" if self._default_deny else ": default_allow"),
            )
        ...
        for rule in matching_rules:
            if destination in rule.blocked_domains or destination in rule.blocked_ips:
                decision = EgressDecision(
                    ...
                    port=port,
                    rule_name=rule.name,
                    reason=f"destination '{destination}' is explicitly blocked",
                )
                self._decisions.append(decision)
                return decision
            if rule.max_payload_bytes > 0 and payload_bytes > rule.max_payload_bytes:
                decision = EgressDecision(
                    ...
                    port=port,
                    rule_name=rule.name,
                    reason=f"payload {payload_bytes}B exceeds max {rule.max_payload_bytes}B",
                )
                self._decisions.append(decision)
                return decision
            dest_allowed = (
                destination in rule.allowed_domains
                or destination in rule.allowed_ips
                or (not rule.allowed_domains and not rule.allowed_ips)
            )
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "allowed": allowed,
            "denied": denied,
            "deny_rate": denied / max(len(self._decisions), 1) * 100,
