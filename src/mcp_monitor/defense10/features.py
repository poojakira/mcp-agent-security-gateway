...
_SECRET_WORDS = re.compile(
    r"\b(secret|password|api[_ ]?key|token|credential|ssh|private[_ ]?key|vault)s?\b", re.I
)
_EXFIL_VERBS = re.compile(
    r"\b(send|leak|dump|exfiltrate|forward|upload|wire|transmit|copy|mirror|redirect)\b", re.I
)
...
_CORP_HINT = re.compile(
    r"\b(meeting|invoice|report|standup|lunch|calendar|review|thanks|rsvp|agenda)\b", re.I
)
