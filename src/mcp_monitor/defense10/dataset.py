...
_EVIL_URLS = [
    ...
    "https://abc.ngrok.io/exfil",
    "https://webhook.site/steal",
    "http://{dom}/collect",
    "https://{dom}/dump",
]
...
def generate(n_per_family: int = 40, seed: int = 42) -> tuple[list[str], list[str]]:
    ...
    for _ in range(n_per_family):
        ...
        mal.append(
            json.dumps(
                {
                    rng.choice(["url", "webhook", "callback", "endpoint"]): rng.choice(
                        _EVIL_URLS
                    ).format(dom=_dom()),
        ...
        ben.append(
            json.dumps(
                {
                    rng.choice(["text", "prompt", "message"]): rng.choice(
                        [
        ...
        ben.append(
            json.dumps(
                {
                    rng.choice(["a", "x"]): rng.randint(1, 999),
