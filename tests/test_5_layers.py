...
class TestProxy:
    ...
    def test_explicit_block_rule(self):
        ...
        p.add_rule(
            ProxyRule(
                name="no_shell", description="", tool_pattern=r"shell", action=ProxyAction.BLOCK
            )
        )
    ...
    def test_redact_rule(self):
        ...
        d = p.intercept(
            {"name": "email.send", "server_id": "s", "arguments": {"to": "x", "bcc": ["hidden"]}}
        )
    ...
    def test_quarantine_list(self):
        ...
        p.add_rule(
            ProxyRule(
                name="q", description="", tool_pattern=r"^sus$", action=ProxyAction.QUARANTINE
            )
        )
    ...
    def test_release_quarantined(self):
        p = InlineProxyGateway()
        p.add_rule(
            ProxyRule(name="q", description="", tool_pattern=r"^q$", action=ProxyAction.QUARANTINE)
        )
...
class TestKernel:
    ...
    def test_dns_blocked(self):
        ...
        e = SyscallEvent(
            server_id="strict", syscall_type=SyscallType.DNS_RESOLVE, details={"domain": "evil.com"}
        )
